...
<header style="background: url('<?php
        echo git_get_option('git_customhead'); ?>')" id="header" class="header">
<?php
    } ?>
<?php
    if (!git_get_option('git_customhead')) { ?>
<header style="background: url('<?php
        echo esc_url( get_template_directory_uri() ); ?>/css/img/header.jpg')" id="header" class="header">
<?php
    } ?><?php
} 
 */?>
...
<?php
//if (git_get_option('git_tmnav_b')) echo '<style type="text/css">#nav-header{background:rgba(0,148,255,.5);background-color: rgba(0,148,255,.5);}</style>'; ?>
<?php
if (git_get_option('git_skin_b') == 'git_red_b') {
	$skin_nom = "#E74C3C";
	$skin_hover = "#D52D1A";
...
if (git_get_option('git_skin_b') == 'git_red_b') {
   ...
   echo '<div id="nav-header" class="navbar">';
} else {
...
<?php
//if (git_get_option('git_bdshare_b')) echo '<style type="text/css"></style>'; 
?>
	...
	<!--logo s-->
	<div class="nav-logo">
		<a href="/"><img title="<?php bloginfo('name'); ?>" alt="<?php bloginfo('name'); ?>" src="<?php echo git_get_option('git_customlogo'); ?>"></a>
	</div>
	<!--logo e-->
	
<ul class="nav site-nav site-navbar">
<?php
echo str_replace('</ul></div>', '', preg_replace('/<div[^>]*><ul[^>]*>/', '', wp_nav_menu(array('theme_location' => 'nav', 'echo' => false))));?>
<li style="float:right;">
	<div class="toggle-search navto-search"><i class="fa fa-search"></i>
